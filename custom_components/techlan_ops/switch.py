...
from ._shared.shared_const import ARMED_CODES
...
async def async_setup_entry(
    ...
    for pku, item in coordinator.data.get("pkus", {}).items():
        for part, details in item.get("parts", {}).items():
            entities.append(
                TechlanPartSwitch(coordinator, int(pku), int(part), details)
            )
...
class TechlanPartSwitch(CoordinatorEntity[TechlanDataUpdateCoordinator], SwitchEntity):
    ...
    async def async_turn_off(self, **kwargs) -> None:
        await self.coordinator.client.async_control_part(
            "disarm", self._pku, self._part
        )
        await self.coordinator.async_request_refresh()
